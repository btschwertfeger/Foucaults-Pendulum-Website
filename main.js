...
function computeFP(input = window.defaultInput) {
    ...
    */
    ...
    let
        x = [...new Array(u.length)].map((elem, index) => u[index].re),
...
function createData(input) {
    let values = []
    for (let i = 0; i < input.x.length; i++) values.push({
        x: input.x[i],
        y: input.y[i]
    })
...
function animatePlot() {
    ...
    if (window.animationIndex >= window.xyData.length) clearInterval(animinterval);
}
